...
class ClipTest extends TestCase
{
    ...
    public function test_store_is_idempotent_on_same_id(): void
    {
        ...
        $this->postJson('/api/clip', $p)->assertOk(); // 200, no duplicate
        $this->assertDatabaseCount('clips', 1);
    }
    ...
    public function test_index_excludes_expired_clips(): void
    {
        ...
        $this->seedClip($u, ['expires_at' => now()->subHour()]); // expired
        $this->seedClip($u);
    ...
    public function test_enforces_max_clips_cap(): void
    {
        ...
        $this->assertDatabaseMissing('clips', ['id' => $firstId]); // the oldest evicted
    }
    ...
    public function test_pinned_clip_survives_cap_and_ttl(): void
    {
        ...
        // A clip that is pinned AND already expired (TTL past).
        $pinned = $this->seedClip($user, ['pinned' => true, 'expires_at' => now()->subDay()]);

        // We exceed the cap with recent clips.
        for ($i = 0; $i < 8; $i++) {
            $this->postJson('/api/clip', $this->clip(['created_at' => now()->addSeconds($i)->toIso8601String()]))
                ->assertSuccessful();
        }

        // The pinned one is not evicted…
        $this->assertDatabaseHas('clips', ['id' => $pinned->id]);
        // …and is still returned by the history despite its expired TTL, at the top.
        $data = $this->getJson('/api/clips')->json('data');
        $this->assertSame($pinned->id, $data[0]['id']);
        $this->assertTrue($data[0]['pinned']);
    }

    public function test_dedup_same_device_duplicate(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);

        $device = (string) Str::uuid();
        $this->postJson('/api/clip', $this->clip(['origin_device_id' => $device, 'dedup_hash' => 'abc123']))->assertCreated();
        $this->postJson('/api/clip', $this->clip(['origin_device_id' => $device, 'dedup_hash' => 'abc123']))->assertOk();
        $this->assertSame(1, Clip::where('user_id', $user->id)->count());
    }

    public function test_dedup_collapses_cross_device_duplicate(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);

        // Same content (hash) from TWO different devices → a single card.
        $this->postJson('/api/clip', $this->clip(['origin_device_id' => (string) Str::uuid(), 'dedup_hash' => 'same']))->assertCreated();
        $this->postJson('/api/clip', $this->clip(['origin_device_id' => (string) Str::uuid(), 'dedup_hash' => 'same']))->assertOk();
        $this->assertSame(1, Clip::where('user_id', $user->id)->count());
    }

    public function test_dedup_bumps_existing_to_top(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);

        // An old clip with a known hash.
        $old = $this->seedClip($user, ['dedup_hash' => 'dup', 'created_at' => now()->subHours(3)]);
        // The same content copied from another device must bump the existing one
        // (created_at refreshed) and not create a duplicate.
        $this->postJson('/api/clip', $this->clip(['dedup_hash' => 'dup']))->assertOk();

        $this->assertSame(1, Clip::where('user_id', $user->id)->count());
        $old->refresh();
        $this->assertTrue($old->created_at->gt(now()->subMinute()), 'created_at doit etre rafraichi');
    }

    public function test_dedup_ignores_expired_clip(): void
    {
        $user = User::factory()->create();
        Sanctum::actingAs($user);

        // An EXPIRED clip with this hash must not capture the dedup: we recreate.
        $this->seedClip($user, ['dedup_hash' => 'old', 'expires_at' => now()->subHour()]);
        $this->postJson('/api/clip', $this->clip(['dedup_hash' => 'old']))->assertCreated();
