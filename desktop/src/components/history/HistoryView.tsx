...
export function HistoryView() {
  ...
  const filtered = useMemo(() => {
    ...
    // Pinned first (stable sort: the server/recency order is otherwise preserved).
    return [...base].sort((a, b) => Number(b.pinned) - Number(a.pinned));
  }, [clips, search, filter]);

  // Keep the selection within bounds when the list changes.
  useEffect(() => {
  ...
  // Initial focus + refocus/reset each time the window reopens (hotkey/tray).
  useEffect(() => {
    focusSearch();
