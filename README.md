...
Presse-papier partagé **self-hosted** et **chiffré de bout en bout** entre un téléphone (iOS/Android) et un ordinateur (macOS, Windows, Linux via Tauri). Copie sur un appareil, retrouve-le sur l'autre — le serveur ne voit jamais que du ciphertext.
...
```
Mobile (React Native)      Serveur Laravel + Reverb          Desktop (Tauri)
...
| `mobile/` | React Native (Expo) | iOS + Android : onboarding, historique live, envoi via partage, images |

La crypto est identique sur toutes les plateformes (Argon2id + AES-256-GCM, même sel partagé) — interopérabilité vérifiée par tests.
...
```
Modules natifs (Argon2, presse-papier, clavier) → build de dev requis (pas Expo Go).
