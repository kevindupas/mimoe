...
/** Width of the actions drawer revealed on right swipe (pin + hide). */
const ACTIONS_W = 132;
/** Beyond this, we consider the gesture intentional rather than a graze. */
const OPEN_THRESHOLD = ACTIONS_W * 0.5;
/** Full left swipe: the card leaves and the clip is deleted. */
const DELETE_THRESHOLD = 110;

export type Filter = "all" | "text" | "image" | "file" | "pinned";

// Image loaded on demand (when the card is rendered), from the disk cache.
function ClipImage({ cfg, clipId, blobId, style, tint }: {
  ...
  // expo-image: natively animates GIF/WebP (unlike RN's <Image> on Android).
  return <Image source={uri} style={style} contentFit="contain" />;
}

/**
 * Row swipable in both directions.
 *
 * Left = delete: destructive gesture isolated on its own side, so that a hurried
 * thumb never reaches it while aiming for a pin. Past the threshold, the card
 * leaves the screen (undo remains possible via the undo bar).
 *
 * Right = pin + hide drawer: revealed and held open rather than triggered on
 * release, so that the action is read before being touched.
 */
function SwipeRow({ children, onDelete, onPin, onHide, pinned, danger, p, t }: {
  ...
  const pan = useMemo(() => PanResponder.create({
    // The horizontal threshold (1.4x the vertical) protects the list scroll:
    // a mostly vertical gesture never takes the responder.
  ...
  return (
    <View style={{ marginBottom: 8 }}>
      {/* Left background: delete. Revealed when the card slides to the left. */}
      <View style={[StyleSheet.absoluteFill, s.deleteBg, { backgroundColor: danger }]}>
        <Ionicons name="trash-outline" size={22} color="#fff" />
      </View>

      {/* Actions drawer, bounded to its width and opaque: with absoluteFill it
          let the delete red show through under accentSoft. */}
...
// Animated generative pattern (morphing shapes) for a hidden card.
const MORPH_BAG = [0, 1, 2, 3, 4, 5, 5, 5, 5, 5, 6, 6, 6]; // weights: empty/faint dominate
...
export default function Home({ p, cfg, clips, refreshing, onRefresh, onSwipeDelete, hidden, onToggleHide, onTogglePin }: {
  ...
  const shown = useMemo(() => {
    ...
    return clips.filter((c) => {
      ...
      // A hidden card stays filterable by type but never by its content:
      // surfacing it on a keyword would defeat the hiding.
  ...
  async function copyClip(c: Clip) {
    // File: no "file clipboard" on mobile → we open the share/save.
  ...
  return (
    <View style={[s.root, { paddingTop: insets.top }]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          // Hidden or not, the card stays swipable: without this a hidden clip
          // could no longer be deleted or pinned.
          <SwipeRow
          ...
          >
            {hidden.has(item.id) ? (
              // Hidden card: field of morphing shapes + eye in the center, tap to reveal.
...
const styles = (p: Palette) => StyleSheet.create({
  ...
  // Overflows by 16px under the card: without this, its rounded corners let the
  // delete red show right at the drawer edge. Square right corners for the same reason.
